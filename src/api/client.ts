import type {
  ...
  ProductionSummary, RecoveryPlan, RegisterAgentInput, AgentRun, AgentEvidence, ImprovementCandidate, ScriptChange
} from '../types/optrane';
import { ensureAccessToken } from './session';
import { OPTRANE_API_BASE } from '../config/optrane';

export const API_BASE = OPTRANE_API_BASE;
...
async function parseError(response: Response, path: string): Promise<ApiError> {
  ...
  try {
    const body = await response.json() as { detail?: string; error?: { message?: string; code?: string } };
...
export async function uploadScriptWithProgress(
  ...
  return new Promise<UploadedScript>((resolve, reject) => {
    ...
    xhr.open('POST', `${API_BASE}/scripts/upload`);
    ...
    xhr.onload = () => {
      ...
      if (xhr.status < 200 || xhr.status >= 300) {
        reject(new ApiError(xhr.status, body?.error?.message ?? body?.detail ?? `Upload failed (${xhr.status})`, '/scripts/upload', body?.error?.code ?? body?.code));
        return;
      }
...
export const api = {
  ...
  eventUrl: (_productionId: string, analysisId: string) => `${API_BASE}/analyses/${analysisId}/events`,
