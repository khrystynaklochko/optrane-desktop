...
export function AgentRunsPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <section className="page">
    ...
    {loading ? <div className="empty-state">Loading governance evidence…</div> : <div className="runs-layout">
      ...
      <div className="panel"><div className="panel-head"><div><span className="eyebrow">EVIDENCE</span><h3>Authorization & actions</h3></div><span className="count-pill">{evidence.length}</span></div>{evidence.length === 0 ? <div className="empty-small">No evidence events recorded yet.</div> : <div className="evidence-list">{evidence.map((item) => {
        const decision = (item.decision ?? (item.success ? 'ALLOW' : 'EVENT')).toUpperCase();
        const decisionClass = decision === 'HOLD_FOR_APPROVAL' ? 'hold' : decision === 'DENY' ? 'deny' : decision === 'ALLOW' ? 'allow' : 'event';
        return <div key={item.id} className="evidence-row"><div className={`decision-dot decision-${decisionClass}`}/><div><b>{item.tool ?? item.eventType}</b><p>{item.summary}</p><small>{new Date(item.createdAt).toLocaleString()}</small></div><span className={`decision-label decision-${decisionClass}`}>{decision.replaceAll('_', ' ')}</span></div>;
      })}</div>}</div>
    </div>}
  </section>;
}
