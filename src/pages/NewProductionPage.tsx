...
export function NewProductionPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  async function create() {
    if (!title || !shootStart || !shootEnd || !file) return onToast('Complete all fields and choose a screenplay file.');
    if (!isScriptFile(file)) return onToast('OPTRANE accepts screenplay PDF, plain text (.txt), or Final Draft (.fdx) files.');
    setBusy(true);
    try {
  ...
  return <section className="page narrow">
    ...
    <label className="dropzone compact"><input type="file" accept="application/pdf,.pdf,.txt,text/plain,.fdx,application/xml" onChange={(e) => setFile(e.target.files?.[0] ?? null)}/><div className="clapper">▰</div><b>{file?.name ?? 'Choose baseline screenplay'}</b><span>{file ? `${(file.size/1024/1024).toFixed(1)} MB` : 'PDF, .txt, or .fdx'}</span></label>
