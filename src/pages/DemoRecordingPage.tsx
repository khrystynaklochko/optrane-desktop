...
export function DemoRecordingPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  const refreshAgents = async (productionId = state.activeProductionId) => {
    ...
    try {
      ...
      setAgents(next);
      if (defaultFleetTypes.every((type) => next.some((agent: ProductionAgent) => agent.agentType === type && agent.status === 'ACTIVE'))) setFleetState('DONE');
      return next;
    } catch { return []; }
  };
  ...
  async function provisionFleet() {
    ...
    try {
      ...
      const missing = defaultFleetTypes.filter((type) => !existing.some((agent: ProductionAgent) => agent.agentType === type && agent.status !== 'REVOKED'));
      if (missing.length) {
      ...
      const ready = defaultFleetTypes.every((type) => after.some((agent: ProductionAgent) => agent.agentType === type && agent.status === 'ACTIVE'));
  ...
  async function openImpactAgent() {
    ...
    try {
      ...
      const agent = current.find((item: ProductionAgent) => item.agentType === 'IMPACT');
  ...
  return <section className="page demo-recording-page">
    ...
    <div className="recording-overview panel">
      ...
      <i>→</i><div><span>LOCATION SCOUT</span><strong>Gemini</strong><small>Parallel citations · graded risks</small></div>
    ...
    <div className="panel recording-script">
      ...
      <ol>
        ...
        <li><b>Location &amp; Clearance Scout</b><span>From an impact finding, run Gemini + Parallel research; show staged spinner, risk cards, and parallelSearchId.</span></li>
        <li><b>Recovery</b><span>Compare A/B/C and approve Plan B — readiness returns to ~91%.</span></li>
        <li><b>Settings</b><span>Integration tiles proving Google Cloud backend/model and Parallel are live.</span></li>
      ...
      <div className="button-row"><button className="ghost" onClick={() => state.setScreen('control')}>Open Control Room</button><button className="ghost" onClick={() => state.setScreen('research')}>Open Location Scout</button><button className="ghost" onClick={() => state.setScreen('recovery')}>Open Recovery</button><button className="ghost" onClick={() => state.setScreen('settings')}>Open Settings</button><button className="ghost" onClick={() => state.setScreen('audit')}>Open Audit</button></div>
    </div>
  </section>;
}
