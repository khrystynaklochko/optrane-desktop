...
export function ScriptRevisionPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  function choose(candidate: File | null) {
    if (!candidate) return;
    if (!candidate.name.toLowerCase().endsWith('.pdf') && candidate.type !== 'application/pdf') return onToast('OPTRANE accepts screenplay PDFs only.');
  ...
  async function upload() {
    if (!file) return onToast('Choose a revised screenplay PDF first.');
    controller.current = new AbortController();
    try {
      setUploadState('UPLOADING');
      const uploaded = await api.uploadScript(state.activeProductionId, file, 'REVISION', setProgress, controller.current.signal);
  ...
  return <section className="page narrow">
    ...
    <label className={`dropzone ${uploadState === 'FAILED' ? 'failed-drop' : ''}`} onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
      <input type="file" accept="application/pdf,.pdf" onChange={(e) => choose(e.target.files?.[0] ?? null)} /><div className="clapper">▰</div><b>{file?.name ?? 'Drop revised screenplay PDF'}</b><span>{file ? `${(file.size/1024/1024).toFixed(1)} MB · ${uploadState}` : 'or click to choose a file'}</span>
    </label>
