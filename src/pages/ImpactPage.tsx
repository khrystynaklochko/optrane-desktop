...
export function ImpactPage() {
  ...
  return <section className="page">
    ...
    {!!state.impacts.length && <>
      <div className="impact-list">{state.impacts.map((x) => <article className="impact-row" key={x.id}><SeverityPill severity={x.severity}/><div><h3>{x.category}</h3><p>{x.reason}</p><small>Production evidence · {x.evidence}</small>{x.evidenceRefs?.some((ref) => ref.source === 'MCP_CLICKHOUSE') && <span className="evidence-source">MCP_CLICKHOUSE</span>}</div><b>{x.status.replaceAll('_',' ')}</b></article>)}</div>
